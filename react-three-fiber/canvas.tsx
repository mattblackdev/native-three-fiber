...
export const Canvas = React.memo(
  ({
    ...
    useEffect(() => {
      ...
      if (
        (state.current.camera as THREE.OrthographicCamera).isOrthographicCamera
      ) {
      ...
      if (ready) {
        ...
        if (updateDefaultCamera) {
          if (
            (state.current.camera as THREE.OrthographicCamera)
    ...
    const intersect = useCallback((event, prepare = true) => {
      ...
      const intersects = defaultRaycaster
        ...
        .filter(item =>
          seen.hasOwnProperty(item.object.uuid)
            ? false
            : (seen[item.object.uuid] = true),
        )
      ...
      for (let intersect of intersects) {
        ...
        while (object) {
          if (object.__handlers)
            hits.push({ ...intersect, object, receivingObject })
    ...
    const handleIntersects = useCallback(
      (event: React.PointerEvent<HTMLDivElement>, fn): [] => {
        prepareRay(event)
    ...
    const handlePointerCancel = useCallback(
      (event: React.PointerEvent<HTMLDivElement>, hits?: []) => {
        ...
        Object.values(hovered.current).forEach(data => {
          if (!hits.length || !hits.find(i => i.object === data.object)) {
            ...
            if (handlers && handlers.pointerOut)
              handlers.pointerOut({ ...data, type: 'pointerout' })
    ...
    return (
      <div
        ...
        // (state.current.captured = intersect(event, false))
        // }
        // On lost capture remove the captured hit
        // onLostPointerCapture={event => (
        // (state.current.captured = undefined), handlePointerCancel(event)
        // )}
        {...rest}
        style={{
          position: 'relative',
      ...
      >
        <canvas
          ref={canvas as React.MutableRefObject<HTMLCanvasElement>}
          style={{ display: 'block' }}
        />
      </div>
    )
